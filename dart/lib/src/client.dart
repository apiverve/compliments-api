...
class ComplimentsValidationException implements Exception {
  final List<String> errors;

  ComplimentsValidationException(this.errors);

  @override
  String toString() => 'ComplimentsValidationException: ${errors.join("; ")}';
}
...
/// Compliment Generator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/compliments?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [adult] - Include flirty/romantic compliments
class ComplimentsClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'adult': ValidationRule(type: 'boolean', required: false),
  };

  ComplimentsClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/compliments',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [ComplimentsValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw ComplimentsValidationException(errors);
    }
  }

  /// Execute a request to the Compliment Generator API.
  ///
  /// Parameters are validated before sending the request.
  Future<ComplimentsResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw ComplimentsException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ComplimentsResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw ComplimentsException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw ComplimentsException('Resource not found');
      } else {
        throw ComplimentsException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is ComplimentsException) rethrow;
      throw ComplimentsException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Compliment Generator API client.
class ComplimentsException implements Exception {
  final String message;

  ComplimentsException(this.message);

  @override
  String toString() => 'ComplimentsException: $message';
}
